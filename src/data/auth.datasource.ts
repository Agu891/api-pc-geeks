...
import AuthRespository from '../core/repositories/auth.repository';
...
export default class AuthDataSource implements AuthRespository {
  public async login(data: AuthLogin): Promise<Result<AuthDto>> {
    ...
    if (!isMatch) {
      return {
        success: false,
        err: new BadRequestError('Email o contraseña invalido'),
      };
    }
  ...
  public async signin(data: AuthSignIn): Promise<Result<AuthDto>> {
    ...
    if (existUser) {
      return {
        success: false,
        err: new BadRequestError('El usuario ya existe'),
      };
    }
